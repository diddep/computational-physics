   ...
   )
{
    ...
    if(task_num == 4)
    {
        N_steps = 1e6; N_discarded_steps = 1e3; alpha = 0.5;
        initial_displacement = 50, d_displacement = 1.24; 
        N_alpha_steps = 200; A = 1.; beta = 1.; is_save = false; // beta from 0.5 to 1
        is_vary_beta = true;
        is_task4 = true;
        printf("Hej!");
    }
    ...
    if(is_task4)
    {
        ...
        for(int bx = 0; bx < N_beta_steps; bx++)
        { 
            if(is_vary_beta)
            {
                double beta_array[11] = {0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1};
                beta = beta_array[bx];
                alpha = 0.2;
            }
            ...
            {   
                ...
                if (ix == N_alpha_steps && bx == N_beta_steps) {is_save = true; }
                E_average = MCMC(N_steps, alpha, d_displacement, R1, R2, is_save);
                ...
                double alpha_result_vector[] = {ix, E_average, alpha, gamma, E_PD_average, beta};
                if(ix == 1 && bx == 0){ open_with_write = true; } else { open_with_write = false; }
                save_vector_to_csv(alpha_result_vector, 6, filename_alpha_results, open_with_write);
                printf("Alpha iteration: %d\n", ix);
            }
            printf("Beta iteration: %d\n", bx);
        }
